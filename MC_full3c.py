...
for i in range(10):
	...
	for j in range(40000,60000):
		if j % 50 == 0:
			np.save('samples/evt_%i_c' %ID,data_array) #Save to disk every 1000 iterations
		...
		try:
			...
			for k in range(8):
			  data_array[j-40000,k] = out[k]
		except:
			data_array[j-40000,8] = 1.
			print("Timeout")
	np.save('samples/evt_%i_c' %ID,data_array)
