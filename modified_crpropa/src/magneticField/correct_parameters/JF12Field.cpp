...
JF12Field::JF12Field(int seed, double rs, double wh, double r0Turb, double z0Turb) {
  ...
  hDisk = regRand.randNorm(0.40,0.0009) * kpc;
	...
	bDisk[1] = regRand.randNorm(3.0,.36) * muG;   // b2
	bDisk[2] = regRand.randNorm(-0.9,.64) * muG;  // b3
	bDisk[3] = regRand.randNorm(-0.8,.09) * muG;  // b4
	bDisk[4] = regRand.randNorm(-2.0,.01) * muG;  // b5
  ...
  //   --- toroidal halo parameters
	bNorth = regRand.randNorm(1.4,.01) * muG;
	bSouth = regRand.randNorm(-1.1,.01) * muG;
  ...
  //   --- X halo parameters
	bX = regRand.randNorm(2.92,.09) * muG;
	...
	bDiskTurb[0] = regRand.randNorm(6.196,5.4289) * muG;  // b1
	bDiskTurb[1] = regRand.randNorm(6.528,2.4964) * muG;  // b2
	bDiskTurb[2] = regRand.randNorm(5.708,1.21) * muG;    // b3
	bDiskTurb[3] = regRand.randNorm(6.528,0.7569) * muG;  // b4
	bDiskTurb[4] = regRand.randNorm(2.656,1.7424) * muG;  // b5
	bDiskTurb[5] = regRand.randNorm(10.28,6.401) * muG;   // b6
	bDiskTurb[6] = regRand.randNorm(16.788,5.7121) * muG; // b7
	bDiskTurb[7] = regRand.randNorm(7.884,19.625) * muG;   // b8

	bDiskTurb5 = regRand.randNorm(5.39,1.932) * muG;      // bint
	zDiskTurb = regRand.randNorm(0.61,0.0016) * kpc;

	bHaloTurb = regRand.randNorm(4.68,1.9321) * muG;      // B0
...
void JF12Field::randomTurbulent(int seed) {
	useTurbulent = true;
	// turbulent field with Kolmogorov spectrum, B_rms = 7.8 and Lc = 51 parsec
	turbulentGrid = new VectorGrid(Vector3d(0.), 512, 2 * parsec);
	initTurbulence(turbulentGrid, 7.8, 15 * parsec, 300 * parsec, -11./3., seed);
}
#endif
