...
for i in range(10):
	...
	for j in range(20000,40000):
		if j % 50 == 0:
			np.save('samples/evt_%i_b' %ID,data_array) #Save to disk every 1000 iterations
			print("Iteration %i / %i" %(j,40000))
		...
		try:
			...
			print("Timeout")
	np.save('samples/evt_%i_b' %ID,data_array)
