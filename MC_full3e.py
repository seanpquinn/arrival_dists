...
for i in range(10):
	...
	for j in range(80000,100000):
		if j % 50 == 0:
			np.save('samples/evt_%i_e' %ID,data_array) #Save to disk every 1000 iterations
			print("Iteration %i / %i" %(j,80000))
		...
		try:
			...
			for k in range(8):
			  data_array[j-80000,k] = out[k]
		except:
			data_array[j-80000,8] = 1.
			print("Timeout")
	np.save('samples/evt_%i_e' %ID,data_array)
